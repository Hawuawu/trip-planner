...
const SEED_CHECKPOINTS: Checkpoint[] = [
  { id: '1', type: 'flight', name: 'JFK → NRT', startTime: '2026-10-01T14:00:00.000Z', endTime: '2026-10-02T17:00:00.000Z', notes: 'JL 005, seat 32A', updatedAt: now },
  { id: '2', type: 'hotel', name: 'Shinjuku Granbell Hotel', startTime: '2026-10-02T15:00:00.000Z', endTime: '2026-10-05T11:00:00.000Z', location: { lat: 35.6938, lng: 139.7034, label: 'Shinjuku, Tokyo' }, updatedAt: now },
  { id: '3', type: 'train', name: 'Shinkansen Tokyo → Kyoto', startTime: '2026-10-05T12:00:00.000Z', endTime: '2026-10-05T14:15:00.000Z', notes: 'Nozomi 37, car 7', updatedAt: now },
  { id: '4', type: 'hotel', name: 'Kyoto Granvia', startTime: '2026-10-05T15:00:00.000Z', endTime: '2026-10-08T11:00:00.000Z', location: { lat: 34.9856, lng: 135.7579, label: 'Kyoto Station' }, updatedAt: now },
  { id: '5', type: 'poi', name: 'Fushimi Inari-taisha', startTime: '2026-10-06T08:00:00.000Z', location: { lat: 34.9671, lng: 135.7727, label: 'Fushimi, Kyoto' }, updatedAt: now },
  { id: '6', type: 'flight', name: 'NRT → JFK', startTime: '2026-10-14T10:00:00.000Z', endTime: '2026-10-14T14:00:00.000Z', notes: 'JL 006', updatedAt: now },
];

const SEED_ALTERNATIVES: Alternative[] = [
  { id: 'a1', type: 'poi', name: 'teamLab Borderless', notes: 'Azabudai Hills, Tokyo', location: { lat: 35.6591, lng: 139.7138, label: 'Azabudai Hills, Tokyo' } },
  { id: 'a2', type: 'poi', name: 'Nishiki Market', location: { lat: 35.0053, lng: 135.7659, label: 'Nishiki, Kyoto' } },
];
...
function saveCp(c: Checkpoint[]) { localStorage.setItem(LS_CP, JSON.stringify(c)); }
...
export class LocalTripRepository implements TripRepository {
  ...
  subscribeToCheckpoints(tripId: string, cb: (c: Checkpoint[]) => void): () => void {
    ...
    cb(this.sortedCp());
    return () => { this.cpSubs.get(tripId)?.delete(cb); };
  }
  ...
  async addCheckpoint(tripId: string, cp: Omit<Checkpoint, 'id' | 'updatedAt'>): Promise<Checkpoint> {
    const saved: Checkpoint = { ...cp, id: `local-${Date.now()}`, updatedAt: new Date().toISOString() };
    saveCp([...loadCp(), saved]);
  ...
  async updateCheckpoint(tripId: string, id: string, changes: Partial<Omit<Checkpoint, 'id' | 'updatedAt'>>): Promise<void> {
    saveCp(loadCp().map(c => c.id === id ? { ...c, ...changes, updatedAt: new Date().toISOString() } : c));
    this.notifyCp(tripId);
  }
  ...
  subscribeToAlternatives(tripId: string, cb: (a: Alternative[]) => void): () => void {
    ...
    cb(loadAlt());
    return () => { this.altSubs.get(tripId)?.delete(cb); };
  }
